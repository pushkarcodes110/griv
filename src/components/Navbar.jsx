    ...
    return (
        <nav className={`fixed top-6 left-1/2 -translate-x-1/2 z-50 w-[90%] max-w-5xl transition-all duration-500 rounded-[2rem] px-6 py-4 flex items-center justify-between ${scrolled
            ...
            <div className="flex items-center gap-2">
                <span className="font-sans font-bold text-xl tracking-tight text-text">
            ...
            <div className="hidden md:flex items-center gap-8">
                {['Systems', 'Process', 'Protocol'].map((link) => (
                    <a key={link} href={`#${link.toLowerCase()}`} className="text-sm font-medium text-textMuted hover:text-text transition-colors hover-lift">
                        {link}
                    </a>
                ))}
            </div>
            ...
            {/* CTA Desktop */}
            <button className="hidden md:inline-flex magnetic-btn bg-surface text-text border border-surface px-6 py-2.5 rounded-full text-sm font-medium hover:border-accent">
            ...
            {mobileMenuOpen && (
                <div className="absolute top-[120%] left-0 w-full bg-background/95 backdrop-blur-xl border border-surface rounded-2xl p-6 flex flex-col gap-4 shadow-2xl md:hidden">
                    {['Systems', 'Process', 'Protocol'].map((link) => (
                        <a key={link} href={`#${link.toLowerCase()}`} onClick={() => setMobileMenuOpen(false)} className="text-lg font-medium text-textMuted hover:text-text">
                            {link}
                        </a>
                    ))}
                    <button className="magnetic-btn w-full mt-4 bg-accent text-background px-6 py-3 rounded-full text-sm font-bold">
